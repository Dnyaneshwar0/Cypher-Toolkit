...
def generate_captcha_flow():
    ...
    # Save image
    image_path = os.path.join(data_folder, "./images/captcha/captcha_test.png")
    with open(image_path, "wb") as f:
        f.write(image_bytes)
    print(f"✅ CAPTCHA image saved at: {image_path}")

    # Save signature & text
    with open(os.path.join(data_folder, "./texts/captcha/captcha_signature.txt"), "w") as f:
        f.write(signature)
    with open(os.path.join(data_folder, "./texts/captcha/captcha_answer.txt"), "w") as f:
        f.write(text)
